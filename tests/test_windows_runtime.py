...
def test_sso_collection_is_timestamped_sso_only_deduplicated_and_private():
    previous = grok_register_ttk.ACCOUNTS_DIR
    previous_path = grok_register_ttk._sso_collection_path
    try:
        with tempfile.TemporaryDirectory() as temp:
            grok_register_ttk.ACCOUNTS_DIR = str(Path(temp) / "accounts")
            grok_register_ttk._sso_collection_path = None
            ...
            assert path.name.startswith("SSO_only_") and path.suffix == ".txt"
            assert path.read_text(encoding="utf-8") == token + "\n"
            if sys.platform != "win32":
                assert stat.S_IMODE(path.stat().st_mode) == 0o600
    finally:
        grok_register_ttk.ACCOUNTS_DIR = previous
        grok_register_ttk._sso_collection_path = previous_path
...
if __name__ == "__main__":
    ...
    test_sso_collection_is_timestamped_sso_only_deduplicated_and_private()
